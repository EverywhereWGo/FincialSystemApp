...
public class AddTransactionViewModel extends ViewModel {
    ...
    /**
     * 添加交易记录（增强版，支持note、remark和imagePath字段）
     * @param type 交易类型：0-支出，1-收入
     * @param amount 金额
     * @param categoryId 分类ID
     ...
     */
    public LiveData<Transaction> addTransaction(int type, double amount, long categoryId, 
                                               String description, Date date, String note, 
                                               String remark, String imagePath) {
        LogUtils.d(TAG, "ViewModel开始添加交易记录（增强版带图片）：type=" + type + ", amount=" + amount + ", categoryId=" + categoryId);
        MutableLiveData<Transaction> result = new MutableLiveData<>();
        ...
        executor.execute(() -> {
            ...
            request.setImagePath(imagePath); // 设置图片路径
            request.setUserId(1); // 使用当前登录用户ID
